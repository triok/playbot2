...
RF_MIN_PCT   = 5.0
MIN_ORDER_USD = 1.0
IMPROVEMENT_THRESHOLD = 3.0  # минимум 10% улучшения WCS для действия
MIN_WINNER_PROFIT_AFTER_INSURANCE = 7.0  # Лидер должен приносить минимум 7% после покупки страховки
...
def choose_label(snap, portfolio, snapshots=None, idx=None, reversal_confirmed=False):
    """
    ...
    for n in get_buy_steps(winner_price, portfolio.budget_left):
        ...
        # Добавляем условие reversal_confirmed сюда:
    ...
    for n in get_buy_steps(loser_price, portfolio.budget_left):
        ...
        # И сюда тоже добавляем reversal_confirmed:
        elif reversal_confirmed and score > current_wcs + IMPROVEMENT_THRESHOLD:
            candidates.append(("RECOVERY_BUY", score, {
    ...
    if loser_price > 0 and loser_price <= cheap_threshold:
        # 1. Если мы УЖЕ находимся в состоянии Risk-Free, страховка не нужна.
        # Мы и так заработаем при любом исходе.
        if is_risk_free(cur_pe, cur_ph):
            pass # Просто идем дальше, не добавляя CHEAP_BUY_LOSER в кандидаты
        else:
            insurance_spend = 2.0
            n_shares = int(insurance_spend / loser_price)
            ...
            if n_shares > 0:
                ...
                # Симулируем результат покупки
                is_rf, score, pe, ph, cost = try_action(portfolio, price_e, price_h, be, bh)
                
                # Определяем PnL лидера (победной стороны) ПОСЛЕ симулированной покупки
                sim_winner_pct = pe if leader_is_entry else ph
                
                # 2. ПРОВЕРКА СТОИМОСТИ: Покупаем страховку только если прибыль лидера 
                # не упадет ниже установленного порога (например, 7%)
...
def label_market(data):
    ...
    # Добавляем переменную для отслеживания подтвержденного разворота
    reversal_confirmed = False

    for idx, snap in enumerate(snapshots):

        # Проверяем, произошел ли разворот, если он еще не был подтвержден ранее
        if not reversal_confirmed:
            reversal_data = detect_reversal(snapshots, idx)
            if reversal_data is not None:
                reversal_confirmed = True
                # Можно вывести лог для отладки: 
                # print(f"Market {data['marketId']}: Reversal confirmed at snap {idx}")

        # Передаем статус разворота в choose_label
        ...
        # Пересчитываем pnl поля на основе ЖИВОГО портфеля (до действия)
        sim_se   = portfolio_snapshot["sim_shares_entry"]
        sim_sh   = portfolio_snapshot["sim_shares_hedge"]
        sim_sp   = portfolio_snapshot["sim_total_spent"]
        if sim_sp > 0:
            sim_pnl_e     = sim_se - sim_sp
            sim_pnl_h     = sim_sh - sim_sp
            sim_pct_e     = sim_pnl_e / sim_sp * 100
            sim_pct_h     = sim_pnl_h / sim_sp * 100
        else:
            sim_pnl_e = sim_pnl_h = sim_pct_e = sim_pct_h = 0.0

        labeled_snapshots.append({
            **snap,
            # Состояние портфеля ДО действия — то, что видела модель
            **portfolio_snapshot,
            # Пересчитанные pnl поля — актуальны для sim портфеля
            "pnl_if_entry_wins":     round(sim_pnl_e, 4),
            "pnl_if_hedge_wins":     round(sim_pnl_h, 4),
            "pnl_pct_if_entry_wins": round(sim_pct_e, 4),
            "pnl_pct_if_hedge_wins": round(sim_pct_h, 4),
            "label":        label,
            "label_score":  score,
            "label_detail": detail,
        })
