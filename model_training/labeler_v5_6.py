...
MIN_ORDER_USD  = 1.0
MAX_SINGLE_BUY_USD = 10.0 # ОГРАНИЧЕНИЕ: не более $10 за одну операцию
...
def choose_label(snap, portfolio):
    ...
    if lose_price > 0 and lose_price <= 0.05:
        ins_cost = 2.0 
        if ins_cost <= portfolio.budget_left:
            return "CHEAP_BUY_LOSER", 400, {"shares": round(ins_cost/lose_price, 2), "cost": ins_cost}

    # 2. ПРОВЕРКА ВЫХОДА В RISK-FREE (BUY_WINNER)
    ...
    if shares_to_rf and shares_to_rf > 0:
        ...
        # Если стоимость выхода в RF приемлема
    ...
    # 3. ПРОВЕРКА RECOVERY_BUY (Порционная докупка)
    if portfolio.budget_left >= MIN_ORDER_USD:
        # Берем либо столько, сколько надо для RF, либо лимит $10
        target_cost = min(portfolio.budget_left, MAX_SINGLE_BUY_USD)
        can_buy = target_cost / win_price
        return "RECOVERY_BUY", 100, {"shares": round(can_buy, 2), "cost": round(target_cost, 2)}
...
def label_market(data):
    ...
    winner_is_hedge = (win_side != entry_out)
    final_profit = p_e if win_side == entry_out else p_h
    return labeled_snapshots, final_profit, portfolio.total_spent, winner_is_hedge
...
def main():
    ...
    for fp in files:
        try:
            ...
            labeled_snaps, m_profit, m_spent, is_hedge_win = label_market(data)
            if not labeled_snaps: continue
            
            # ВАЖНО: сохраняем флаги для вьювера
            data["snapshots"] = labeled_snaps
            data["winner_is_hedge"] = is_hedge_win
