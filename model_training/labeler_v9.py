...
RF_MIN_PCT   = 5.0
MIN_ORDER_USD = 1.0 
IMPROVEMENT_THRESHOLD = 10.0 # Для глубокого минуса
PRICE_CAP = 0.82 # Максимальная цена покупки

REVERSAL_CONFIRM_SNAPS  = 2     
REVERSAL_MIN_STRENGTH   = 0.15  

def get_grid_order_size(reversal_strength, budget_left):
    if reversal_strength >= 0.40: return min(budget_left * 0.20, 5.0)
    elif reversal_strength >= 0.25: return min(budget_left * 0.12, 3.0)
    elif reversal_strength >= 0.15: return min(budget_left * 0.08, 2.0)
    else: return min(budget_left * 0.05, 1.0)
...
def choose_label(snap, winner_is_hedge, snapshots=None, idx=None):
    ...
    # Динамический порог: если минус небольшой, бот становится активнее
    effective_threshold = 2.0 if current_wcs > -15.0 else IMPROVEMENT_THRESHOLD
    ...
    # 1. CHEAP_BUY_LOSER (Страховка)
    cheap_threshold = get_cheap_threshold(snap["seconds_left"])
    if loser_price > 0 and loser_price <= cheap_threshold and budget_left > 2.0:
    ...
    # 2. УМНОЕ УСРЕДНЕНИЕ (RF и RECOVERY)
    for n in [1, 2, 3, 5, 8, 10, 15]:
        for target in ["winner", "loser"]:
            p = winner_price if target == "winner" else loser_price
            if p > PRICE_CAP: continue # Не покупаем дороже 0.82
            
            be = n if (target == "winner" and leader_is_entry) or (target == "loser" and not leader_is_entry) else 0
            bh = n if be == 0 else 0
            is_rf, score, pe, ph, cost = try_action(snap, be, bh)
            
            if is_rf:
                candidates.append(("BUY_" + target.upper(), score + 1000, {"shares": n, "cost": cost, "type": "RF"}))
            elif score > current_wcs + effective_threshold:
                candidates.append(("RECOVERY_BUY", score, {"shares": n, "cost": cost, "target": target}))
    ...
    if snapshots and idx:
        ...
        if reversal:
            ...
            if nl_price < 0.75: # Сетка работает чуть строже по цене
                order_usd = max(get_grid_order_size(strength, budget_left), MIN_ORDER_USD)
                n_shares = int(order_usd / nl_price)
                is_rf, score, _, _, cost = try_action(snap, 
                    buy_entry=(n_shares if new_leader == "entry" else 0),
                    buy_hedge=(n_shares if new_leader != "entry" else 0))
                if cost > 0:
                    candidates.append(("GRID_BUY_LEADER", score + 100, {"shares": n_shares, "cost": cost}))
...
def label_market(data):
    ...
    for idx, snap in enumerate(snapshots):
        virtual_snap = {
            **snap,
            "shares_entry": v_shares_e, "shares_hedge": v_shares_h,
            "total_spent": v_spent, "budget_left": v_budget
        }
