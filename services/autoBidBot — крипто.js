...
import { nowTime, getTickSizeForOrder, saveOrder, getSymbolFromKeyword, priceThresholds, priceThresholds5m, isBotDisabledNow, arbitrageTestFlag, isCryptoMarket } from "./utils.js"; 
    ...
    async function tick(opportunities) {
      ...
      for (const opp of opportunities) {
        ...
        if(isCryptoMarket(opp)){
          ...
          // это что бы не удаляли спортивные и игры из кэша

          // const marketId = opp.id;
          // let state = marketStates.get(marketId) || {}; 

          // // Теперь безопасно проверяем фазу
          // if (!state.phase) {
          //   state.phase = "other_market";
          //   state.botResult1 = 1;
            
          //   // Записываем обновленный объект обратно в хранилище
...
//       const MAX_MARKET_BUDGET_XRP_SOL = 52;
//       if(opp.keyword == 'xrp' || opp.keyword == 'solana'){
//         availableFunds = MAX_MARKET_BUDGET_XRP_SOL - I_total;
//       }
  ...
  // }
    ...
    async function sendArbitrageOrder(orderData, opp){
      ...
      if (result?.success && result?.orderID) {
        ...
        logText = `[${nowTime()}] ❌ New arbitrage order failed`, result.info;
        pushMarketLog(opp.id, logText);
        onSignal?.({ type: 'bidding', opp, text: logText }); 
        console.log(logText);
        state.phase = "positions_recalculate";
