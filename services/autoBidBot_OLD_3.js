...
export function createAutoBidBot({ onSignal, placeOrder, placeOrderSell, executeSpreadTrade, client, placeTestOrder, placeArbitrageOrder }) {
    ...
    async function startArbitrage(opp) {
      ...
      if (state.phase === "waiting_first_match") {
        ...
        if (arbitrageTestFlag) {
          const cheapOutcome = opp.outcomes.find(o => Number(o.price) <= 0.34);
        ...
        if (state.subStatus === "waitingRiskFreeOrder") {
          ...
          try {
            ...
            // ⏱ Тайм-аут — отменяем ордер и идём в recalculate
              ...
              await cancelOrder(riskFreeOrder.orderId, client);
        ...
        if (openArbitrageOrders.length > 0) {
          ...
          if (arbitrageOrder.status === 'OPEN' && now - arbitrageOrder.timeoutStart >= 12_000) {
            ...
            if (cancelResult?.canceled && cancelResult.canceled.length > 0) {
              ...
              // ← снимаем флаг GTC если это был он
      ...
      if(state.phase === "positions_recalculate"){
        ...
        try {
          ...
          if(result.action){
            ...
            if (order && result.action.order_type === 'GTC') {
              state.activeGTCOrderId = order.orderId;       // ← запомнили
     ...
     * @param {Function}[params.onSignal]
     */
// рабочая но с тюнингом который все испротил
    // function recalculate({
    //   positions,
    //   entry,
    //   hedge,
...
//   if (S_A <= 0 || S_B <= 0) { log(`❌ I_total = 0, пропускаем`); return null; }
...
//     if (c.price >= 0.92) continue; // слишком дорого
...
//   const leader   = P_A >= LEADER_THRESHOLD ? entry  : (P_B >= LEADER_THRESHOLD ? hedge  : null);
//   const underdog = P_A >= LEADER_THRESHOLD ? hedge  : (P_B >= LEADER_THRESHOLD ? entry  : null);
//   const P_leader   = P_A >= LEADER_THRESHOLD ? P_A : P_B;
//   const P_underdog = P_A >= LEADER_THRESHOLD ? P_B : P_A;

//   if (leader) {
//     const Profit_leader   = leader.assetId === entry.assetId ? Profit_A : Profit_B;
//     const Profit_underdog = underdog.assetId === entry.assetId ? Profit_A : Profit_B;
//     const targetProfit    = I_total * 0.05;
//     const neededSize      = Profit_leader >= targetProfit
//       ? null
//       : (targetProfit - Profit_leader) / (1 - P_leader);

//     if (Profit_leader < 0) {
//       // Лидер убыточен — докупаем его первым
//       if (neededSize !== null) {
//         const leaderSize = enforceMinAmount(P_leader, Math.ceil(neededSize * 100) / 100);
//         const sim = simulate(leader.assetId, P_leader, leaderSize);
//         log(`📈 Лидер ${leader.name} убыточен, докупаем x${leaderSize} до +5%`);
//         return ret({
//           action: {
//             type:       'buy',
...
//       }
//     } else {
//       // Лидер в профите — сначала страховка лузера
//       if (P_underdog <= 0.09 && Profit_underdog < I_total) {
//         // Считаем максимум который можно потратить не снизив лидера ниже +2%
//         const minLeaderProfit = I_total * 0.02;
//         const maxSpend        = Profit_leader - minLeaderProfit;
//         const maxSize         = maxSpend / P_underdog;

//         if (maxSize <= 0) {
//           log(`🛡️ Страховка ${underdog.name} пропущена — лидер уже близко к +2%`);
//         } else {
//           const underdogSize = enforceMinAmount(P_underdog, Math.floor(maxSize * 100) / 100);
//           const simU = simulate(underdog.assetId, P_underdog, underdogSize);
//           log(`🛡️ Лузер ${underdog.name} @ ${P_underdog}, страховка x${underdogSize} (лидер останется >= +2%)`);
//           return ret({
//             action: {
//               type:       'buy',
...
//               price:      P_underdog,
//               order_type: 'FOK',
//               reason:     `underdog insurance @ ${P_underdog} (leader=${leader.name} @ ${P_leader}, keeping leader >= +2%)`,
//               nextTurn,
//               sim:        simU,
//             }
//           });
...
//       // Лидер в профите но ещё не достиг 5% — докупаем
//       if (neededSize !== null) {
//         const leaderSize = enforceMinAmount(P_leader, Math.ceil(neededSize * 100) / 100);
//         const sim = simulate(leader.assetId, P_leader, leaderSize);
//         log(`📈 Лидер ${leader.name} в профите, докупаем x${leaderSize} до +5%`);
//         return ret({
//           action: {
//             type:       'buy',
//             assetId:    leader.assetId,
